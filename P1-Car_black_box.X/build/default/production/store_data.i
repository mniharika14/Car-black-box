...
void store_data(unsigned char write[10]) {
    ...
    if (data_lim == 100) {
        ...
        if(clearing_log == 2){
        for (int i = 0; i < 90; i++) {
            unsigned char temp = read_external_eeprom(shift_index++);
            write_external_eeprom(data_lim++, temp);
        }
        for (int i = 0; i < 10; i++) {
        write_external_eeprom(data_lim++, write[i]);
    }
}
